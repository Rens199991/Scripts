...
$targetRgName = Get-AutomationVariable -Name "avd-vm-rgname"
$rgName = Get-AutomationVariable -Name "avd-image-rgname"
$vaultName = Get-AutomationVariable -Name "avd-kv-name"
$deployAvdJsonUrl = Get-AutomationVariable -Name "avd-vm-arm-url"
$deployAvdParamJsonUrl = Get-AutomationVariable -Name "avd-vm-armparam-url"
...
Invoke-WebRequest -Uri $deployAvdJsonUrl -OutFile deploy.avd.json -UseBasicParsing
Invoke-WebRequest -Uri $deployAvdParamJsonUrl -OutFile deploy.avd.params.json -UseBasicParsing

Write-Output "Reading ARM Template"
if ($PSVersionTable.PSVersion.Major -eq 5) {
    $armparamobject = Get-Content ".\deploy.avd.params.json" | ConvertFrom-Json | ConvertTo-Hashtable
}
else {
    $armparamobject = Get-Content ".\deploy.avd.params.json" | ConvertFrom-Json -AsHashtable
}
...
$deploymentTest = $null
$deploymentTest = Test-AzResourceGroupDeployment -ResourceGroupName $targetRgName -TemplateFile ".\deploy.avd.json" -TemplateParameterObject $bogusparameterobject
if (($null -eq $deploymentTest) -or ($deploymentTest.Count -eq 0)) {
    Write-Output "Kicking off the deployment"
    $deployResult = New-AzResourceGroupDeployment -ResourceGroupName $targetRgName -TemplateFile ".\deploy.avd.json" -TemplateParameterObject $bogusparameterobject -Mode Incremental -Name $deploymentName
