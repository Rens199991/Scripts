...
If(-not(Test-Path -Path "$($env:ProgramData)\CXN\Scripts\RemoveWindowsUpdateKeys"))
	{
    New-Item -itemtype "directory" -path "$($env:ProgramData)\CXN\Scripts\RemoveWindowsUpdateKeys"
    Set-Content -Path "$($env:ProgramData)\CXN\Scripts\RemoveWindowsUpdateKeys\RemoveWindowsUpdateKeys.ps1.tag" -Value "Installed"
    }
