...
If (-not(Test-Path -Path "$($env:ProgramData)\CXN\Scripts\SetTimeZoneDynamically"))
    {   
    New-Item -itemtype "directory" -path "$($env:ProgramData)\CXN\Scripts\SetTimeZoneDynamically"
    Set-Content -Path "$($env:ProgramData)\CXN\Scripts\SetTimeZoneDynamically\SetTimeZoneDynamically.ps1.tag" -Value "Installed"
    }
