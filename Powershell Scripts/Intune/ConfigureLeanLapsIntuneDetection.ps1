if(-not(Test-Path -Path "$($env:ProgramData)\CXN\Scripts\ConfigureLeanLapsD"))
    {
    New-Item -itemtype "directory" -path "$($env:ProgramData)\CXN\Scripts\ConfigureLeanLapsD"
    Set-Content -Path "$($env:ProgramData)\CXN\Scripts\ConfigureLeanLapsD\ConfigureLeanLapsD.ps1.tag" -Value "Installed"
    }
