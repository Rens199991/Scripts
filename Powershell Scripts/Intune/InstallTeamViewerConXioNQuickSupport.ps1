...
If(-not(Test-Path -Path "$($env:ProgramData)\CXN\Scripts\InstallTeamViewerConXioNQuickSupport"))
	{
    New-Item -itemtype "directory" -path "$($env:ProgramData)\CXN\Scripts\InstallTeamViewerConXioNQuickSupport"
    Set-Content -Path "$($env:ProgramData)\CXN\Scripts\InstallTeamViewerConXioNQuickSupport\InstallTeamViewerConXioNQuickSupport.ps1.tag" -Value "Installed"
    }
