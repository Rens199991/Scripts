...
If(-not(Test-Path -Path "$($env:ProgramData)\CXN\Scripts\RemoveTeamsandEdgeCopyFromDestkopR"))
        {
        New-Item -itemtype "directory" -path "$($env:ProgramData)\CXN\Scripts\RemoveTeamsandEdgeCopyFromDestkopR"
        Set-Content -Path "$($env:ProgramData)\CXN\Scripts\RemoveTeamsandEdgeCopyFromDestkopR\RemoveTeamsandEdgeCopyFromDestkopR.ps1.tag" -Value "Installed"
        }      
